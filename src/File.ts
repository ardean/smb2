...
import { FileInfoClass, InfoType } from "./packets/SetInfo";
...
class File extends EventEmitter {
  ...
  async open(path: string, options: OpenOptions = {}) {
    ...
    const response = await this.tree.request(PacketType.Create, {}, {
      ...
      desiredAccess: typeof options.desiredAccess === "number" ?
        ...
        FilePipePrinterAccessMask.ReadData,
      fileAttributes: FileAttributeMask.Normal,
      shareAccess:
    ...
    this.fileSize = (response.body.endOfFile as Buffer).readBigInt64LE(0);
    this.isOpen = true;
  ...
  async create(path: string) {
    await this.open(path, {
      desiredAccess: FilePipePrinterAccessMask.WriteData,
  ...
  async write(content: Buffer | string) {
    ...
    for (let index = 0; index < chunkCount; index++) {
      ...
      await this.tree.request(PacketType.Write, {}, {
        fileId: this._id,
        buffer: chunk,
        offset: offsetBuffer
      });
  ...
  async read() {
    ...
    for (let index = 0; index < chunkCount; index++) {
      ...
      const response = await this.tree.request(PacketType.Read, {}, {
        fileId: this._id,
        length: lengthBuffer,
        offset: offsetBuffer
      });
